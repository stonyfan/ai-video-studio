...
PROBE_DIR = ROOT / "_scene_probe"        # 已有 50% 帧
EXTRA_DIR = ROOT / "_scene_extra"        # 新抽 25%/75%
TRIPLET_DIR = ROOT / "_scene_triplet"

FRAME_W = 360  # 每帧缩到 360 宽


def log(msg):
    print(f"[triplet] {msg}", flush=True)
...
def make_triplet(scene):
    ...
    for f in [f25, f50, f75]:
        ...
        im = Image.open(f)
        # 统一高度
        h = int(im.width * 640 / FRAME_W) if im.width != FRAME_W else 640
        # 实际上 scale=360:-1 已经按比例缩放，这里再统一为 640 高（竖屏）
        im = im.resize((FRAME_W, 640))
        imgs.append(im)
